(() => {
	...
	const t = () => localStorage.getItem("theme"), a = () => {
		...
		r && (document.querySelectorAll("[data-bs-theme-value]").forEach(e => {
			e.classList.remove("active"), e.setAttribute("aria-pressed", "false")
		}), r.classList.add("active"), r.setAttribute("aria-pressed", "true"), a && r && (a.textContent, r.dataset.bsThemeValue), a = e, e = document.querySelector(".theme-icon-active"), a = document.querySelector(`[data-bs-theme-value="${a}"] .theme-icon`), e && a && (e.innerHTML = a.outerHTML), t) && r.focus()
	});
